...
internal class Nienna_JobDriver_HealingTouch : JobDriver
{
    ...
    private void DoHealingEffect(Pawn patient)
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            var arg_BB_0 = patient.health.hediffSet.hediffs.Where(hediff => hediff is Hediff_Injury);

            bool ArgBb1(Hediff injury)
            {
                return injury.Part == rec;
            }

            foreach (var hediff in arg_BB_0.Where(ArgBb1))
            {
                var current = (Hediff_Injury)hediff;
                if (num2 <= 0)
                {
                    continue;
                }
