    ...
    protected override void Impact(Thing hitThing, bool blockedByShield = false)
    {
        ...
        for (var k = 0; k < wallLengthMax; k++)
        {
            ...
            if (!hasWall)
            {
                ...
                {
                    if (!cellList[i].def.EverHaulable && cellList[i] is not Pawn)
                    {
                        if (cellList[i].def.altitudeLayer is AltitudeLayer.Building or AltitudeLayer.Item
                            or AltitudeLayer.ItemImportant)
                        {
